...
class EnquiriesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create enquiry" do
    assert_difference('Enquiry.count') do
      post enquiries_url, params: { enquiry: { email: @enquiry.email, message: @enquiry.message, name: @enquiry.name } }
    end
  ...
  test "should update enquiry" do
    patch enquiry_url(@enquiry), params: { enquiry: { email: @enquiry.email, message: @enquiry.message, name: @enquiry.name } }
